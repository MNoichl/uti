...
def calculate_utils_n_agents(omega, n, game_matrix): # omega = tr-probability, n = number agents, m = number action per agent, game_matrix is matrix with r cells and n-tuples per cell
    pre_set = calculate_pre_set_n_agents(n, game_matrix)
    shaped_utils = np.zeros(len(pre_set)) # for now this is a one dimensional array TODO: Check what format gambit needs to compute NE
    for index, profile_list in enumerate(pre_set): # for a list of tuples
        util = 0
